...
public class Main {
    ...
    public static void tmbhBarang() throws IOException {
        ...
        String tmbhJenis = utk_input.readLine();
        ...
        BarangGrosir gros = new BarangGrosir(tmbhNama, tmbhJenis, tglMasuk, tglKeluar, jmlKardus, jmlBarang);//objek gros
    ...
    public static void baca() throws IOException {
        if (grosir.size() == 0) {
            ...
            for (int i = 0; i < grosir.size(); i++) {
                System.out.println((i + 1 ) + "\tNama Barang \t\t: " + grosir.get(i).namaBarang);
                System.out.println("\tJenis Barang \t\t: " + grosir.get(i).jenisBarang);
                System.out.println("\tTanggal Masuk Barang \t: " + grosir.get(i).tanggalMasuk);
                System.out.println("\tTanggal Keluar Barang \t: " + grosir.get(i).tanggalKeluar);
                System.out.println("\tBanyak Dus \t\t: " + grosir.get(i).banyakDus);
                System.out.println("\tBanyak Barang per Dus \t: " + grosir.get(i).barangPerdus);
                System.out.println("\tTotal Barang (pcs) \t: " + grosir.get(i).banyakDus * grosir.get(i).barangPerdus);
        }
        }
    }
    ...
    public static void updateBarang() throws IOException {
        ...
        String newNama = utk_input.readLine();
        System.out.print("Masukan Jenis Barang       : ");
        String newJenis = utk_input.readLine();
        System.out.print("Masukan Tanggal Masuk      : ");
        String newTmasuk = utk_input.readLine();
        System.out.print("Masukan Tanggal Keluar     : ");
        String newTkeluar = utk_input.readLine();
        System.out.print("Banyak Dus                 : ");
        int newDus = Integer.parseInt(utk_input.readLine());
        System.out.print("Banyak Barang per Dus      : ");
        int newBperdus = Integer.parseInt(utk_input.readLine());

        grosir.get(indexx).namaBarang = newNama;
        grosir.get(indexx).jenisBarang = newJenis;
        grosir.get(indexx).tanggalMasuk = newTmasuk;
        grosir.get(indexx).tanggalKeluar = newTkeluar;
        grosir.get(indexx).banyakDus = newDus;
        grosir.get(indexx).barangPerdus = newBperdus;
